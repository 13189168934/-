...
$(function() {
    ...
    setTimeout(function() {
        let span = $(`<span class="nihao" style="width: 80px;height:28px">亲，在吗?</span>`);
        $('.nihao:last').after(span);
    }, 13000)

    setTimeout(function() {
        let span = $(`<span class="nihao" style="width: 80px;height:28px">亲，还在吗?</span>`);
        $('.nihao:last').after(span);
    }, 16000)
    ...
    $(".dialogue button").click(function() {
        if ($('textarea').val().trim() != '') {
			...
			<span class="touxiang"></span>
			 `);
            li.children('.wenzi').text($("textarea").val());
            console.log();
            $('.user').append(li);

            $("textarea").val('');
        }
        // console.log($(".content_t ul").height());
        if ($(".content_t ul").height() > 350) {
            $(".content_t").css('overflow-y', 'scroll');
            $(".content_t").scrollTop($(".content_t ul").height());
        }
    })
    $('textarea').keydown(function(e) {
        if (e.keyCode == 13) {
            $(".dialogue button").click();
        }
    })
